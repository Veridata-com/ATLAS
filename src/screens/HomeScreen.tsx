...
export const HomeScreen: React.FC = () => {
    ...
    const loadSavedGuides = async () => {
        ...
        setSavedGuides(saved.slice(0, 3)); // Preview of 3 most recent
    };
    ...
    const handleSearch = async (query: string) => {
        ...
        const { canSearch, remainingSearches } = await canPerformSearch();

        if (!canSearch) {
            Alert.alert(
                'Search Limit Reached',
                'You\'ve used all your searches today. Upgrade to Premium for unlimited searches.',
                [{ text: 'OK' }]
            );
            setSearchQuery('');
            return;
